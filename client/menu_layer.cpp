...
MenuLayer::MenuLayer(Master *master)
    : Subsystem(master)
{
    init();
}
...
bool MenuLayer::init()
{
    ...
    CCSprite* pSprite = CCSprite::spriteWithFile("menu_bg.png");
    ...
    cocos2d::CCLabelTTF *start_l = CCLabelTTF::labelWithString("Start", "Arial", 28);
    ...
    cocos2d::CCLabelTTF *exit_l = CCLabelTTF::labelWithString("Exit", "Arial", 28);
    exit_l->setPosition( ccp(size.width / 3 * 2, size.height / 3 * 2 - 52) );
    this->addChild(exit_l, 1);

    // create menu, it's an autorelease object
    CCMenu* pMenu = CCMenu::menuWithItems(exit, start, NULL);
    pMenu->setPosition( CCPointZero );
    this->addChild(pMenu, 1);

    _input_password.create_input(this, ccp(size.width/2, size.height/2 - bg_size.height / 2 + 24), CCSize(200, 24), CCTextAlignmentLeft, "<password>", "Arial", 20);

    _input_name.create_input(this, ccp(size.width/2, size.height/2 - bg_size.height / 2 + 64), CCSize(200, 24), CCTextAlignmentLeft, "<user name>", "Arial", 20);

    cocos2d::CCLabelTTF *caption = CCLabelTTF::labelWithString("Menu", "Arial", 36);
...
{
    _touch_begin = touch->locationInView(touch->view());	
...
{
    CCPoint touch_end = touch->locationInView(touch->view());
