...
GameLayer::GameLayer(Master *master)
    : Subsystem(master)
{
    init();
}
...
bool GameLayer::init()
{
    ...
    CCMenuItemImage *left_button = CCMenuItemImage::itemFromNormalImage(
                                        "test_button.png",
                                        "test_button_pressed.png",
                                        &master().subsystem<Game>(),
                                        menu_selector(Game::left_fire) );
    left_button->setPosition( ccp(_size.width /2 - 60, 20) );

    CCMenuItemImage *right_button = CCMenuItemImage::itemFromNormalImage(
                                        "test_button.png",
                                        "test_button_pressed.png",
                                        &master().subsystem<Game>(),
                                        menu_selector(Game::right_fire) );
    right_button->setPosition( ccp(_size.width / 2 + 60, 20) );

    // create menu, it's an autorelease object
    _pMenu = CCMenu::menuWithItems(pMenuItem, left_button, right_button, NULL);
    _pMenu->setPosition( CCPointZero );
    _pMenu->setIsVisible(false);
    this->addChild(_pMenu, 1);


    _tip = CCLabelTTF::labelWithString("", "Arial", 36);
    _tip->setPosition( ccp(_size.width/2, _size.height/2) );
    _tip->setIsVisible(false);
    this->addChild(_tip, 1);

    _good_tip = CCLabelTTF::labelWithString("", "Arial", 36);
    _good_tip->setPosition( ccp(_size.width/2, _size.height/2) );
    _good_tip->setColor(ccc3(100, 100, 255));
    _good_tip->setIsVisible(false);
    this->addChild(_good_tip, 1);

    _bad_tip = CCLabelTTF::labelWithString("", "Arial", 36);
    _bad_tip->setPosition( ccp(_size.width/2, _size.height/2) );
    _bad_tip->setColor(ccc3(255, 100, 100));
    _bad_tip->setIsVisible(false);
